...
import { assert, assertSequence } from '@yamf/test'
...
export function testIsJsonString() {
  assertSequence([
    '{"key":"value"}',
    '[1,2,3]',
    'not json',
    '{incomplete'
  ].map(t => () => isJsonString(t)),
    r => r === true,
    r => r === true,
    r => r === false,
    r => r === false
...
export function testDetectFromBuffer() {
  assertSequence([
    Buffer.from('test'),
    'string',
    { key: 'value' }
  ].map(t => () => detectFromBuffer(t)),
    r => r === 'application/octet-stream',
    r => r === null,
    r => r === null
  )
}

export function testDetectContentType() {
  assertSequence([
    '{"key":"value"}',
    ...
    'plain text here',
    Buffer.from('binary')
  ].map(t => () => detectContentType(t)),
    t => t === 'application/json',
    t => t === 'text/html',
    t => t === 'text/html', // assumes html if no (.xml) url is provided
    t => t === 'text/plain',
...
export function testGetServiceAddresses() {
  const state = createRegistryState()
  state.services.set('myService', new Set(['loc1', 'loc2', 'loc3']))
  
  assert(getServiceAddresses(state, 'myService'),
    r => Array.isArray(r),
    r => r.length === 3,
...
export function testSelectServiceLocationRandom() {
  const state = createRegistryState()
  state.services.set('myService', new Set(['loc1', 'loc2', 'loc3']))
  
  assert(selectServiceLocation(state, 'myService', 'random'),
    r => typeof r === 'string',
    r => ['loc1', 'loc2', 'loc3'].includes(r)
  )
}

export function testSelectServiceLocationRoundRobin() {
  resetRoundRobinState()
  const state = createRegistryState()
  state.services.set('myService', new Set(['loc1', 'loc2', 'loc3']))
  
  assert(Array.from({ length: 6 }, () => selectServiceLocation(state, 'myService', 'round-robin')),
    r => r.includes('loc1') && r.includes('loc2') && r.includes('loc3'),
